...
object MergeJob extends App {
  override def main(args: Array[String]): Unit = {
    ...
    (xml \ "inputs").foldLeft[(RDD[(String, (ModelInput, List[String]))])](null) { (rdd, node) =>
      ...
      .foreachPartition { iter =>
        ...
        iter.foreach {
          case (key, (mi, ids)) =>
            ...
            put.addColumn("cf".getBytes, "serverIp".getBytes, s"${mi.serverIp.getOrElse("")}".getBytes)
            table.put(put)
        }
        table.close
      }
...
//
//    val table = new HTable(conf,"internet_security")
//    val get = new Get("haha".getBytes())
//    val result = table.get(get)
//    println(new String(result.value()))
//  }
//}